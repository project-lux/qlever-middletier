        ...
        if query.field == self.name_field:
            ...
            for w in words:
                ...
                if self.calculate_scores:
                    strips.append(
                        Triple(cfvar, "textSearch:score", f"?score_{self.counter}{wx}")
                    )
                wx += 1
...
COALESCE(?score_text_{self.counter}{wx}, 0) * {self.text_weight}",
    ...
    def make_sparql_ref(self, query, scope, w, wx, optional):
        ...
        if self.portal is None:
            opt1.add_triples(
                [Triple(query.var, f"lux:{scope}Any/lux:primaryName", fldvar)]
            )
        else:
            fldvar2 = f"{fldvar}P"
        ...
        if self.calculate_scores:
            strips.append(
                Triple(cfvar, "textSearch:score", f"?score_refs_{self.counter}{wx}")
            )
    ...
    def make_sparql_anywhere(self, query, scope, w, wx, optional):
        ...
        if self.calculate_scores:
            strips.append(
                Triple(cfvar, "textSearch:score", f"?score_text_{self.counter}{wx}")
            )
        svc.add_triples(strips)
        opt1.add_nested_graph_pattern(svc)
        ...
        if self.calculate_scores:
            strips.append(
                Triple(cfvar, "textSearch:score", f"?score_name_{self.counter}{wx}")
            )
        svc.add_triples(strips)
